...
;
    ...
    {
        // Define a helper function in this scope
        auto obtain = [this](const Eval& value)
                      {
                            if constexpr (std::is_same_v<Value, Scalar>) {
                                static_cast<void>(this); // suppress clang warning
        ...
        const auto& intQuants = ebosSimulator.model().intensiveQuantities(cell_idx, /*timeIdx=*/ 0);
        auto fs = intQuants.fluidState();
        const auto p = scaleFunction(obtain(this->getPerfCellPressure(fs)), xmin[0], xmax[0], feature_min, feature_max);
        // geometrical part of WI
        const auto analytical_PI_scaled = Value(scaleFunction(log10(analytical_PI), xmin[1], xmax[1], feature_min, feature_max));
        ...
        std::cout << "total_inj_gas_scaled: " << tot_inj_gas << std::endl;
        ...
        std::cout << "pressure_scaled: " << p << std::endl;
        std::cout << "pressure: " << obtain(this->getPerfCellPressure(fs))<< std::endl;

        std::cout << "WI: " << pow(10.0, unscaleFunction(out.data_[0],${ymin}, ${ymax}, target_min, target_max)) << std::endl;
        std::cout << "WI log: " << unscaleFunction(out.data_[0],${ymin}, ${ymax}, target_min, target_max) << std::endl;
        std::cout << "WI_scaled: " << getValue(out.data_[0]) << std::endl;

        // restrict output to valid values -> otherwise the simulator crashes in the
        // first time steps.
        // const auto out_data = std::max(std::min(out.data_[0], 0), 1);
        if (getValue(out.data_[0]) < target_min) {
            return pow(10.0,${ymin});
        }
        else if (getValue(out.data_[0]) > target_max) {
            return pow(10.0, ${ymax});
        }
        else {
            return pow(10.0, unscaleFunction(out.data_[0],${ymin}, ${ymax}, target_min, target_max));
        }
    }
