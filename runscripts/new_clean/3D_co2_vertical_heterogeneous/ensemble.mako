...
% for perm in perms: # z-perm is a tenth of the (anisotropic) xy-perm
PERMXY${loop.index} ${perm} PERMZ${loop.index} ${perm / 10} PORO${loop.index} ${POROSITY} THIC${loop.index} ${HEIGHT/NUM_LAYERS}
% endfor
...
${INJECTION_TIME} 1e-1 1e-1 1 ${INJECTION_RATE/6} # Divided by 6, since the model runs on a cake of 60°