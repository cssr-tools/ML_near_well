...
features: np.ndarray = np.array(
    ensemble.extract_features(
        data,
        keywords=["PRESSURE", "PERMX", "FLOWATI+", "WBHP:INJ0"],
        keyword_scalings={
            "PRESSURE": units.BAR_TO_PASCAL,
            "WBHP:INJ0": units.BAR_TO_PASCAL,
        },
    )
)[..., ::3, :, :]
...
# bhp is only available for the last well segment. Calculate the well pressure of all
# other cells by substracting hydrostatic pressure.
bhps: np.ndarray = features[..., 3].reshape(
    features.shape[0],
    features.shape[1],
    runspecs_ensemble["constants"]["NUM_ZCELLS"],
    -1,
)[..., -1, 0, None] - formulas.hydrostatic_fluid(
    runspecs_ensemble["constants"]["SURFACE_DENSITY"],
    np.linspace(
        runspecs_ensemble["constants"]["HEIGHT"],
        0,
        runspecs_ensemble["constants"]["NUM_ZCELLS"],
    ).reshape(1, 1, runspecs_ensemble["constants"]["NUM_ZCELLS"]),
)  # ``shape = (runspecs_ensemble["npoints"], num_timesteps/3, num_layers)
