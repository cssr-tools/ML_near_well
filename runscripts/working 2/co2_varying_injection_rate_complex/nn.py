...
from runspecs import trainspecs_3 as trainspecs
...
FEATURE_TO_INDEX: dict[str, int] = {
    ...
    "injection_rate_upper": 9,
    "injection_rate": 10,
    "injection_rate_lower": 11,
    "radius": 12,
    "total_injected_volume": 13,
    "PI_analytical": 14,
}
...
def restructure_data(
    ...
    7. Injection rate - upper neighbor
    8. Injection rate - cell
    9. Injection rate - lower neighbor
    10. radius
    ...
    if trainspecs["architecture"] == "fcnn":
    ...
    elif trainspecs["architecture"] in ["rnn", "gru", "lstm"]:
        ensemble.store_dataset(
            # Bring time axis to the front, flatten all intermediate axes, swap back
            # batch and time axes.
            new_features.swapaxes(0, 1)
            .reshape(new_features.shape[1], -1, new_features.shape[-1])
            .swapaxes(0, 1),
            targets.swapaxes(0, 1)
            .reshape(new_features.shape[1], -1)
            .swapaxes(0, 1)[..., None],
            new_data_dirname,
        )
...
def reload_data(
    ...
    num_timesteps: int = math.ceil((45 * 10) / step_size_t)
    ...
    ds: tf.data.Dataset = tf.data.Dataset.load(str(data_dirname))
    features, targets = next(iter(ds.batch(batch_size=len(ds)).as_numpy_iterator()))
    # Depending on the network architecture, the data is structured differently.
    if trainspecs["architecture"] == "fcnn":
        features = features.reshape(
            -1, num_timesteps, num_layers, num_xcells, num_features
        )
        targets = targets.reshape(-1, num_timesteps, num_layers, num_xcells, 1)
    elif trainspecs["architecture"] in ["rnn", "gru", "lstm"]:
        # Time axis was swapped to the front before all other axes were flattened (in
        # ``restructure_data``). We repeat this process
        features = (
            features.swapaxes(0, 1)
            .reshape(num_timesteps, -1, num_layers, num_xcells, num_features)
            .swapaxes(0, 1)
        )
        targets = (
            targets.swapaxes(0, 1)
            .reshape(num_timesteps, -1, num_layers, num_xcells, 1)
            .swapaxes(0, 1)
        )
...
def plot_member(
    ...
    trainspecs: dict[str, Any],
    plotted_value: Literal["p_w", "WI", "WI_log"] = "WI",
    ...
    if x_axis == "radius" and trainspecs["architecture"] in ["rnn", "gru", "lstm"]:
    ...
    for num_comp, color in zip(
        ...
        if trainspecs["architecture"] == "fcnn":
            ...
            # TODO: some more variability for radius
            input = feature_member[num_comp, fixed_index][None, ...]
...
def main():
    ...
    elif trainspecs["architecture"] == "rnn":
        model = nn.get_RNN(len(trainspecs["features"]), 1, units=50, activation="tanh")
    elif trainspecs["architecture"] == "lstm":
        model = nn.get_LSTM(len(trainspecs["features"]), 1, units=50, activation="tanh")
    elif trainspecs["architecture"] == "gru":
        model = nn.get_GRU(len(trainspecs["features"]), 1, units=50, activation="tanh")
    ...
    features, targets = reload_data(
        runspecs_ensemble, trainspecs, new_data_dirname, step_size_t=10, num_xcells=14
    )
    for i in range(5):
        plot_member(
            features,
            ...
            model,
            nn_dirname,
            nn_dirname / f"WI_vs_time_member_{i}_radius_value_{5}",
            trainspecs,
            x_axis="time",
            final_time=60.0,
            fixed_index=5,
