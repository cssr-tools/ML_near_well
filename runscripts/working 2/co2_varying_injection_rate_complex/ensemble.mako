...
"""Define the injection values"""<% inj_rates = [(context.kwargs[f"inj_rate_{i}"]  / 6) for i in range(3)] %>
...
% for i in range(3):
15 1e-1 1e-1 1 ${inj_rates[i]} # Divided by 6, since the model runs on a cake of 60°
5 1e-1 1e-1 1 0 # Shut well for 5 days
% endfor