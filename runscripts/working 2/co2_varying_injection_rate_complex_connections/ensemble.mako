...
SWI{i} 0. SNI{i} 0.0 KRW{i} 1 KRN{i} 1 PRE{i} 5000 NKRW{i} 2 NKRN{i} 2 HNPE{i} 2 THRE{i} 1e-4 IGN{i} 0
% endfor
...
"""Define the injection values"""<% inj_rates = [(context.kwargs[f"inj_rate_{i}"]  / 6) for i in range(5)] %><% inj_shutwells_1 = [(context.kwargs[f"INT_shut_wells_{i}"] * 5) for i in range(5)] %><% inj_shutwells_2 = [(context.kwargs[f"INT_shut_wells_{i}"] * 5 + 1) for i in range(5)] %><% inj_shutwells_3 = [(context.kwargs[f"INT_shut_wells_{i}"] * 5 + 2) for i in range(5)] %><% inj_shutwells_4 = [(context.kwargs[f"INT_shut_wells_{i}"] * 5 + 3) for i in range(5)] %><% inj_shutwells_5 = [(context.kwargs[f"INT_shut_wells_{i}"] * 5 + 4) for i in range(5)] %>
"""injection time [d], time step size to write results [d], maximum time step [d], injected fluid (0 wetting, 1 non-wetting), injection rates [kg/day], shut connections (i_0 - i_n)"""
% for i in range(5):
${INJECTION_TIME} 1e-1 1e-1 1 ${inj_rates[i]} ${inj_shutwells_1[i]} ${inj_shutwells_2[i]} ${inj_shutwells_3[i]} ${inj_shutwells_4[i]} ${inj_shutwells_5[i]} # Divided by 6, since the model runs on a cake of 60°
% endfor