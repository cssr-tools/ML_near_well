...
def plot_member(
    ...
    if permeability_index is not None:
        permeability_member: np.ndarray = features[member, ..., permeability_index]
    else:
        permeability_member = np.full_like(
            features[member, ..., permeability_index], math.nan
        )
    ...
    if comparison == "layers":
        ...
        permeability_member = np.swapaxes(permeability_member, 0, 1)
    ...
    if x_axis == "time":
        ...
        permeability_member = np.swapaxes(permeability_member, 1, 2)
        if plotted_value == "p_w":
    ...
    for num_comp, color in zip(
        ...
        ax.scatter(
            ...
            label=rf"{comparison} {num_comp}: data ${plotted_value}$, , $\mathbf{{k}}:"
            + rf" {permeability_member[num_comp, fixed_index, 0]:.2f}\,"
            + rf" {plotted_values_units['perm']}$",
            color=color,
        )
        ax.plot(
            x_values,
    ...
    ax.set_title(
        ...
        + f" pressure layer 0 time step 0 {pressure_member[0, fixed_index, 0] * units.PASCAL_TO_BAR} bar"
    )
