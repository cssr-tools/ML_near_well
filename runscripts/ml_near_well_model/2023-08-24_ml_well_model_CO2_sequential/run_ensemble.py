...
initial pressures, initial temperatures, permeabilities and injection schedules and
construct a dataset containing cell pressures, injection schedule and cell center radii
as features and well indices as targets.
...
Features:
    A 
        ...
        5. injection rate [m^3/day]: Injection rate of the next injection cycle.
        4. injection time [d]: Runtime of the next injection cycle.
        5. radius [m]: Cell center radii.

Targets:
    1. WI [[m^4*s/kg]]: Calculated at the given radius at the end of the given injection
       cycle.
...
ensemble_path = os.path.join(dirpath, runspecs.ENSEMBLE_DIR)
os.makedirs(ensemble_path, exist_ok=True)


# Path to OPM, flow and co2brinepvt.
OPM_ML: str = "/home/peter/Documents/2023_CEMRACS/opm_ml"
CO2BRINEPVT: str = os.path.join(OPM_ML, "/build/opm-common/bin/co2brinepvt")
FLOW: str = os.path.join(OPM_ML, "/build/opm-simulators/bin/flow")
FLAGS = (
...
full_ensemble = np.stack(
    [
        ...
        init_temperatures_v.flatten(),
        permeabilities_v.flatten(),
        schedules_v.reshape(-1, schedules_v.shape[-1]),
    ],
    axis=-1,
)
...
# Create a pyopmnearwell input file for each ensemble member.
mytemplate = Template(filename=os.path.join(dirpath, "ensemble_member.mako"))
for i, member in enumerate(ensemble):
    ...
    with open(
        os.path.join(ensemble_path, f"ENSEMBLE_MEMBER_{i}.DATA"),
        "w",
        encoding="utf8",
    ) as file:
...
for i in range(round(runspecs.NPOINTS / runspecs.NPRUNS)):
    os.system(
        f"{FLOW}"
        + f" {os.path.join(ensemble_path, f'ENSEMBLE_MEMBER_{runspecs.NPRUNS*i}.DATA')}"
        + f" --output-dir={os.path.join(ensemble_path, f'results{runspecs.NPRUNS*i}')} {FLAGS} & "
        f"{FLOW}"
        + f" {os.path.join(ensemble_path, f'ENSEMBLE_MEMBER_{runspecs.NPRUNS*i+1}.DATA')}"
        + f" --output-dir={os.path.join(ensemble_path, f'results{runspecs.NPRUNS*i+1}')} {FLAGS} & "
        f"{FLOW}"
        + f" {os.path.join(ensemble_path, f'ENSEMBLE_MEMBER_{runspecs.NPRUNS*i+2}.DATA')}"
        + f" --output-dir={os.path.join(ensemble_path, f'results{runspecs.NPRUNS*i+2}')} {FLAGS} & "
        f"{FLOW}"
        + f" {os.path.join(ensemble_path, f'ENSEMBLE_MEMBER_{runspecs.NPRUNS*i+3}.DATA')}"
        + f" --output-dir={os.path.join(ensemble_path, f'results{runspecs.NPRUNS*i+3}')} {FLAGS} & "
        f"{FLOW}"
        + f" {os.path.join(ensemble_path, f'ENSEMBLE_MEMBER_{runspecs.NPRUNS*i+4}.DATA')}"
