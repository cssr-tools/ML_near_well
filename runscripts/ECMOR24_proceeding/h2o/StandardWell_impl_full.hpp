...
namespace Opm
{
    ...
    void
    StandardWell<TypeTag>::
    computePerfRate(const Simulator& simulator,
    ...
    /*!
    * \brief Calculate the ML well index.
    *
    * Loads model, collects & scales input features, applies the model, and returns the
    * ML well index.
    */
    template<typename TypeTag>
    template<class Value>
    ...
    StandardWell<TypeTag>::wellIndexEval(
        ...
        return config_.template unscaleAndInverseOutput<Value>("WI", output.data_[0]);
    }

    /*!
    * \brief Construct the input tensor based on the ML model type.
    *
    * Collects & scales input features and constructs the tensor.
    */
    template<typename TypeTag>
    template<class Value>
    ML::Tensor<Value>
