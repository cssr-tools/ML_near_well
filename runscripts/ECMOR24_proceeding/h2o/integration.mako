...
${FLOW} --MLNearWellConfigFile=${MLNEARWELLCONFIGFILE} --linear-solver-reduction=1e-5 --relaxed-max-pv-fraction=0 --newton-max-iterations=50 --newton-min-iterations=5 --tolerance-mb=1e-7 --tolerance-wells=1e-5 --relaxed-well-flow-tol=1e-5 --use-multisegment-well=false --enable-tuning=true --enable-opm-rst-file=true --linear-solver=cprw --enable-well-operability-check=false --min-time-step-before-shutting-problematic-wells-in-days=1e-99

"""Set the model parameters"""
co2store no_disgas_no_diffusion ${RUN_NAME}    #Model (co2store/h2store/co2eor/saltprec) and name of the template file (see src/pyopmnearwell/templates/)
tensor3d 0                          #Grid type (radial/cake/cartesian2d/cartesian/cpg3d/coord2d/coord3d/tensor2d/tensor3d) and size (theta[in degrees]/theta[in degrees]/width[m]/anynumber(the y size is set equal to the x one))
${RESERVOIR_SIZE} ${INT_HEIGHT}     #Reservoir dimensions [m] (Length and height)
${GRID_SIZE} 1 2                    #Number of x- and z-cells [-] and exponential factor for the telescopic x-gridding (0 to use an equidistance partition)
${2*WELL_RADIUS} 0 1                #Well diameter [m], well transmiscibility (0 to use the computed one internally in Flow), and remove the smaller cells than the well diameter
...
0 5 ${int(INT_HEIGHT)}              #Activate perforations [-], number of well perforations [-], and length [m]
...
${INJECTION_TIME} ${REPORTSTEP_LENGTH} 1e-1 0 ${INJECTION_RATE}
