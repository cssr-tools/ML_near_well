        ...
        if (drawdown > 0)  {
            ...
            if (!Base::ml_wi_filename_.empty()) {
                ...
                if constexpr (std::is_same_v<Value, EvalWell>) {
                    WI = this->extendEval(wellIndexEval(perf, ebosSimulator, Tw));
                } else {
                    WI = wellIndexEval(perf, ebosSimulator, Tw);
                }
...
;
                  ...
                  ) const 
    {
        ...
        // neural network
        const auto k = Value(scaleFunction(connection.connection.Kh() /
            connection.connectionLength(), xmin[1], xmax[1], feature_min, feature_max));
        // cell height - unit [m]
        ...
        const auto injection_rate_per_second { 500000  / 86000 };
